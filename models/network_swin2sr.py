...
class SwinTransformerBlock(torch.nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=torch.nn.GELU, norm_layer=torch.nn.LayerNorm, pretrained_window_size=0):
...
class BasicLayer(torch.nn.Module):
    ...
    def _init_respostnorm(self):
        for blk in self.blocks:
            torch.nn.init.constant_(blk.norm1.bias, 0)
            torch.nn.init.constant_(blk.norm1.weight, 0)
            torch.nn.init.constant_(blk.norm2.bias, 0)
            torch.nn.init.constant_(blk.norm2.weight, 0)
...
class RSTB(torch.nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if resi_connection == '1conv':
            ...
            self.conv = torch.nn.Sequential(torch.nn.Conv2d(dim, dim // 4, 3, 1, 1),
...
class Upsample_hf(torch.nn.Sequential):
    """Upsample module.
    Args:
        scale (int): Scale factor. Supported scales: 2^n and 3.
        num_feat (int): Channel number of intermediate features.
...
class Swin2SR(torch.nn.Module):
    ...
    def __init__(self, img_size=64, patch_size=1, in_chans=3,
        ...
        if self.upsampler == 'pixelshuffle_hf':
            self.layers_hf = torch.nn.ModuleList()
            for i_layer in range(self.num_layers):
                layer = RSTB(dim=embed_dim,
                             input_resolution=(patches_resolution[0],
                                               patches_resolution[1]),
        ...
        if resi_connection == '1conv':
            ...
            # to save parameters and memory
            self.conv_after_body = torch.nn.Sequential(torch.nn.Conv2d(embed_dim, embed_dim // 4, 3, 1, 1),
                                                 torch.nn.LeakyReLU(negative_slope=0.2, inplace=True),
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            self.conv_before_upsample = torch.nn.Sequential(torch.nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                                                      torch.nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = torch.nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
        elif self.upsampler == 'pixelshuffle_aux':
            self.conv_bicubic = torch.nn.Conv2d(num_in_ch, num_feat, 3, 1, 1)
            self.conv_before_upsample = torch.nn.Sequential(
                torch.nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                torch.nn.LeakyReLU(inplace=True))
            self.conv_aux = torch.nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            self.conv_after_aux = torch.nn.Sequential(
                torch.nn.Conv2d(3, num_feat, 3, 1, 1),
                torch.nn.LeakyReLU(inplace=True))
            self.upsample = Upsample(upscale, num_feat)
            self.conv_last = torch.nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            ...
            self.conv_last = torch.nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            self.conv_first_hf = torch.nn.Sequential(torch.nn.Conv2d(num_feat, embed_dim, 3, 1, 1),
                                                      torch.nn.LeakyReLU(inplace=True))
            self.conv_after_body_hf = torch.nn.Conv2d(embed_dim, embed_dim, 3, 1, 1)
            self.conv_before_upsample_hf = torch.nn.Sequential(
                torch.nn.Conv2d(embed_dim, num_feat, 3, 1, 1),
                torch.nn.LeakyReLU(inplace=True))
            self.conv_last_hf = torch.nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            bicubic = torch.nn.functional.interpolate(x, size=(H * self.upscale, W * self.upscale), mode='bicubic', align_corners=False)
